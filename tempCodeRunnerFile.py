...
def ndcg_at_k(relevant_docs, retrieved_docs, k):
    dcg = 0.0
    idcg = sum(1 / math.log2(i + 2) for i in range(min(k, len(relevant_docs))))  # Ideal DCG
    for i in range(min(k, len(retrieved_docs))):
        if retrieved_docs[i] in relevant_docs:
            dcg += 1 / math.log2(i + 2)  # Calculate DCG
    return dcg / idcg if idcg > 0 else 0

# Search functionality for multiple queries
...
def search():
    ...
    if request.method == "POST":
        query_input = request.form["query"]  # Get the user query input
        query_list = query_input.split(",")  # If multiple queries are comma-separated

        for query in query_list:
            query = query.strip()  # Clean up extra spaces
            ...
            if not top_docs_with_scores:
                message = f"No results matched for '{query}'."
            else:
                results.append({
                    "query": query,
                    "docs": top_docs_with_scores  # Store all relevant results
                })

                # Automatically determine relevant documents from the output
                relevant_docs = [filename for filename, _ in top_docs_with_scores if any(term in filename.lower() for term in tokenized_query)]

                retrieved_docs = [filename for filename, _ in top_docs_with_scores]

                # Evaluate metrics based on all relevant docs
                precision = precision_at_k(relevant_docs, retrieved_docs, k=len(relevant_docs))
                recall = recall_at_k(relevant_docs, retrieved_docs, k=len(relevant_docs))
                map_score = average_precision(relevant_docs, retrieved_docs)
                ndcg = ndcg_at_k(relevant_docs, retrieved_docs, k=len(relevant_docs))

                # Print metrics for each query in the terminal
                print(f"Query: '{query}'")
                print(f"Precision (All Relevant): {precision:.4f}")
                print(f"Recall (All Relevant): {recall:.4f}")
                print(f"MAP (All Relevant): {map_score:.4f}")
                print(f"nDCG (All Relevant): {ndcg:.4f}")

        # Store results in session to display in the HTML
...
def article(filename):
    ...
    try:
        ...
        # Get previous search results from session
        results = session.get('results', None)
        return render_template("article.html", filename=filename, content=content, results=results)
    except FileNotFoundError:
